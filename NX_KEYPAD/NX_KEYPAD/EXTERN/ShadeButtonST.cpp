...
DWORD CShadeButtonST::OnDrawBackground(CDC *pDC, LPCRECT pRect)
{
	...
	pDC->Draw3dRect(pRect, RGB(0, 0, 0), RGB(0, 0, 0));
...
{
	//m_dNormal.Draw(pDC->GetSafeHdc(),10,10);
	//CButtonST::OnDrawBorder(pDC, pRect);

	CRect rect;
	rect.left	= pRect->left;
	rect.top	= pRect->right;
	rect.right	= pRect->right;
	rect.bottom = pRect->bottom;

	pDC->Draw3dRect(pRect, RGB(0, 0, 0), RGB(0, 0, 0));

	CButtonST::OnDrawBorder(pDC, &rect);

	return BTNST_OK;
} // End of OnDrawBorder

// void CShadeButtonST::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
// {
// 	CButtonST::OnDrawItem(nIDCtl, lpDrawItemStruct);
//}