...
#endif
struct DLL_COMM_EXPORTS SockAddrIn : public SOCKADDR_IN {
public:
	SockAddrIn() { Clear(); }
	...
	//bool    operator==(const SockAddrIn& sin) { return IsEqual( sin ); } /**< error C2678 */
	bool    operator==(const SockAddrIn& sin) const { return IsEqual( sin ); } /**< ok */
...
struct DLL_COMM_EXPORTS stMessageProxy
{
  SockAddrIn address;
  BYTE byData[BUFFER_SIZE];
};

class DLL_COMM_EXPORTS CSocketComm
{
public:
    CSocketComm();
    virtual ~CSocketComm();
...
// SocketComm - data
protected:
    volatile  HANDLE      m_hComm;        // Serial Comm handle
...
protected:
    ...
    static DWORD WINAPI SocketThreadProc(LPVOID pParam);

public:
	bool m_bIsMsgRecive;
	BYTE m_Recv[256];
	void ClearRecive(){ memset( m_Recv, NULL, 256); m_bIsMsgRecive = false; }
};
