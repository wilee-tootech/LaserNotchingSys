...
public:
	...
	std::vector< TDefCmdList > m_vPwrList;
	std::vector< TDefCmdList > m_vEtchListFly; //BJBD
	UINT		m_uTotalRepeat;			   //BJBD
	BOOL		m_bRotationDone;
	UINT        m_uStepCircleNumber;    //BJBD 
	UINT		m_dMultiflier;

	TDefExecuteStatus  m_ExeStatus;
	 
	UINT		m_uCurrentStepCircle;
	BOOL        m_bYouCanDoEtch; 
	//BOOL        m_bListDone;
	//UINT        m_nListDoneID;
	//BOOL        m_bLastOneDone;
...
public:
	...
	//void SetOurBufferListDone(BOOL bListDone);//BJBD
	//BOOL GetOurBufferListDone(); //BJBD
	void SetStepCircleNum(int nStepCircleNum);

	UINT GetStepCircleNum();
	// Version 
	UINT GetDllVer();
	UINT GetHexVer();
	UINT GetRtcVer();
	...
	// Manual Load Lists
	void SetScannerAngle(double dAngle); //BJBD 2017022
	...
	BOOL MakeCmdListLine( int nListNo = 1); //BJBD  
	void GetListStat();
	...
	void SetJumpMarkSpeed( double dJump, double dMark, BOOL bCtrl=FALSE );
