...
BOOL CXMLFile::SaveXMLElement(FILE* file, CXMLElement* lpXMLElement)
{
	...
	//		{
	//			if(strParsedTagName.Find("SimulBaseTabDist") > -1)
	//			{
	...
	//				st_Tap_Data.clear();
	//				st_Tap_Data = deviceData->Get_TapPosition(nTapIndex);
	//				
	//				if(st_Tap_Data.bReverse == false)
	//					str = _T("0");
	//				else 
	//					str = _T("0");

	//				
	//				lpXMLElement->SetElementName((LPSTR)(LPCTSTR)str);
	//				

	//			}
	//			else if(strParsedTagName.Find("SimulTabReverse") > -1)
	//			{
	//				CDeviceManager *pDeviceManager = pDeviceManager->Instance();
	//				CDeviceData *deviceData = pDeviceManager->GetDeviceData();
	//				st_Tap_PositionData st_Tap_Data;
	...
	//			}
	//			else if(strParsedTagName.Find("SimulTabOffset") > -1)
	//			{
	//				CDeviceManager *pDeviceManager = pDeviceManager->Instance();
	//				CDeviceData *deviceData = pDeviceManager->GetDeviceData();
	//				st_Tap_PositionData st_Tap_Data;
	...
	//				st_Tap_Data = deviceData->Get_TapPosition(nTapIndex);
	//				
	//				str.Format("%.03f",st_Tap_Data.dOffset);
	//				lpXMLElement->SetElementName((LPSTR)(LPCTSTR)str);
	//			}
	//			else if(strParsedTagName.Find("SimulTabPosition") > -1)
	//			{
	//				//m_st1_Tap_Position.dPosition = atof(strTemp);
	...
	//				st_Tap_Data = deviceData->Get_TapPosition(nTapIndex);
	//				
	//				str.Format("%.03f",st_Tap_Data.dPosition);
	//				lpXMLElement->SetElementName((LPSTR)(LPCTSTR)str);
	...
	//			lpXMLElement->GetCurrentChild();
	//			LPTSTR lpszElementName2 = lpXMLElement->GetElementName();
	//			
	//			//CString strTemp(LPCSTR(lpszElementName2));
	//			str = LPCSTR(lpszElementName2);

	//			LPTSTR lpszAttributeValue2 = lpXMLElement->GetValue();
	//			LPTSTR lpszValue = lpXMLElement->GetValue();
	//			iNameLen = (int)_tcslen(lpszElementName2);
	//			
	//			fwrite(lpszElementName2, sizeof(BYTE), iNameLen, file);
	//			fwrite(&szCRLF, sizeof(BYTE), 1, file);
...
BOOL CXMLFile::ParseXMLElement(CXMLElement* lpXMLElement, DWORD dwStartOffset, DWORD dwEndOffset)
{
	...
	if (m_lpData != NULL)
	{
				...
				case MOD_CLS:
				{
					...
					if (dwFlags & FLG_OPT)
					{
						...
						else if (_tcsncmp(lpName, lpCloseTagName, iNameLength) == 0)
						{
							...
							strParsedTagName = lpCloseTagName;
		...
		if (bPlainText)
		{
			...
			CString strTemp;
			strTemp.Format("%s", lpszElementName);

			int n = strParsedTagName.Find("SimulTabReverse");


			//2018.10.02 jjsjong
			if(strParsedTagName.Find("SimulTabReverse") > -1)
			{
				if(strTemp.Find("False") > -1 )  m_st1_Tap_Position.bReverse = false;
				else m_st1_Tap_Position.bReverse = true;
				
			}
			else if(strParsedTagName.Find("SimulTabOffset") > -1)
			{
				m_st1_Tap_Position.dOffset = atof(strTemp);
			}
			else if(strParsedTagName.Find("SimulTabPosition") > -1)
			{
				m_st1_Tap_Position.dPosition = atof(strTemp);
				...
				//2018.10.04 jjsjong Test Debug
				m_st1_Tap_Position.dPosition = m_st1_Tap_Position.dPosition;
				//m_st1_Tap_Position.dPosition = m_st1_Tap_Position.dPosition/4;
				if(m_st1_Tap_Position.dPosition > 0)
					deviceData->Add_Tap_Position(m_st1_Tap_Position);
...
void CXMLFile::ClearTapIndex()
{
	nTapIndex = 0;
}


void CXMLElement::SetElementName(LPTSTR lpszValue)
{
	// Check for valie value
	if (lpszValue != NULL)
	{
		...
		// Set element value
		int iValueLen = (int)_tcslen(lpszValue);
		m_lpszName = (LPTSTR)malloc((iValueLen+1)*sizeof(_TCHAR));
		_tcsncpy(m_lpszName, lpszValue, iValueLen);
		m_lpszName[iValueLen] = '\0';
	}
}
