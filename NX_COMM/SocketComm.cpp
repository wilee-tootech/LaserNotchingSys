...
const DWORD DEFAULT_TIMEOUT = 200L;
...
#else
            strcpy_s(strName, strlen(strHost) + 30,strHost);
            return true;
#endif
        }
    }
...
#else
                strcpy_s(strAddress, strlen(strHost),strHost);
                return true;
#endif
            }
        }
...
bool CSocketComm::CreateSocketEx(LPCTSTR strHost, LPCTSTR strServiceName, int nFamily, int nType, UINT uOptions /* = 0 */)
{
    ...
    // Create a Socket that is bound to a specifmic service provide
    // nFamily: (AF_INET)
...
bool CSocketComm::ConnectTo(LPCTSTR strDestination, LPCTSTR strServiceName, int nFamily, int nType)
{
    ...
    if (INVALID_SOCKET != sock)
    {
        // Associate a local address with the socket
        SockAddrIn sockAddr;
        if (false == sockAddr.CreateFrom(NULL, _T("0"), nFamily))
        {
            closesocket( sock );
            return false;
        }
...
bool CSocketComm::WatchComm()
{
	...
	if (!IsStart())
    {
        ...
        {
            //HANDLE hThread;
            DWORD uiThreadId = 100;

		
			m_hThread = CreateThread(NULL, 0, CSocketComm::SocketThreadProc, this, CREATE_SUSPENDED, &uiThreadId);
...
void CSocketComm::StopComm()
{
    ...
    if (IsStart())
    {
		...
		{
			TerminateThread(m_hThread, 1000L);
		}
		*/
		if(m_hThread!=NULL)
		{
...
// NOTES:
///////////////////////////////////////////////////////////////////////////////
DWORD WINAPI CSocketComm::SocketThreadProc(LPVOID pParam)
{
