...
void CLabel::ScreenCapture(CDC* pDC)
{
	...
	// 현재 컨틀롤의 크기를 얻어서..
	//			pDC에다가 현재 컨트롤의 Frame을 포함한 윈도우를 그대로 복사한다..
	////////////////////////////////////
	...
	// 현 컨트롤의 Client영역을 담을 memDC설정..
	CDC memDC;
	memDC.CreateCompatibleDC(pDC);

	// 현 컨트롤의 Client영역을 담을 비트맵 설정..
	CBitmap bmpImage;
	bmpImage.CreateCompatibleBitmap(pDC, rc.Width(), rc.Height());
	memDC.SelectObject(&bmpImage);

	// memDC에다가 현재 컨트롤을 화면을 담는다..
	Paint(&memDC);

	// pDC에다가 현재 Client영역의 memDC를 복사한다..
